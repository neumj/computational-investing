...
def log180Buys(histPriceDF,futDtRng,cash,eqWts,logFile):
    ...
    for i in range(0,len(histPriceDF['close'].keys())):
        ...
        beginPrice = histPriceDF['close'][sym][futDtRng[0]:(futDtRng[1] + dt.timedelta(days=1))][0]
        numShares = math.floor((eqWts[i] * cash[0]) / beginPrice)
        buyDtStr = str(histPriceDF['close'][sym][futDtRng[0]:futDtRng[1]].index[0])
...
def log180Sells(histPriceDF,curDtRng,curPos,logFile):
    for i in range(0,len(curPos.keys())):
        ...
        lastIdx = len(histPriceDF['close'][sym][curDtRng[0]:(curDtRng[1] + dt.timedelta(days=1))]) - 1
        sellPrice = histPriceDF['close'][sym][curDtRng[0]:(curDtRng[1] + dt.timedelta(days=1))][lastIdx]
        cashOut = numShares * sellPrice
        print cashOut
        sellDtStr = str(histPriceDF['close'][sym][curDtRng[0]:(curDtRng[1] + dt.timedelta(days=1))].index[lastIdx])
...
##Script
logFile = "C:\\Users\\Ben\\Documents\\ci\\hw\\MarketTest\\Test180_02.txt"
##Market simulation.
...
for j in range(0,(len(sampleDates)-1)):
    ...
    curPos = log180Buys(priceDF,drPlus,initCash,secWts,logFile)
    log180Sells(priceDF,dtRng,curPos,logFile)
